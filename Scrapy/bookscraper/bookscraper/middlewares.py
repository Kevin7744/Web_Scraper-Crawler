...
class ScrapeOpsFakeUserAgentsMidddleware:
    ...
    # get a random user agent from the list being returned
    def _get_random_user_agent(self):
        random_index = randint(0, len(self.user_agents_list) - 1)
        return self.user_agents_list[random_index]
    
    # Check user to see if active or inactive
    ...
    # Sets the header to a random user agent
    def process_request(self, request, spider):
        random_user_agent = self._get_random_user_agent()
        request.headers['User-Agent'] = random_user_agent
    
        # Just a confirmation that a user-agent has been added
        print("********* NEW HEADER ATTACHED*********")
        print(request.headers['User-Agent'])



# Create a fake browser header.
